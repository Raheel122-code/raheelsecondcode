...
class ApiClient {
  ...
  Future<T> get<T>(String endpoint, {Map<String, dynamic>? queryParams}) async {
    try {
      ...
      throw ApiException(code: 500, message: e.toString(), type: 'ServerError');
    }
  }
      ...
      if (formData != null) {
        ...
        for (var entry in formData.entries) {
          if (entry.value is File ||
            ...
            // Handle file upload
            final filePath =
                entry.value is File ? (entry.value as File).path : entry.value;
  ...
  Future<http.MultipartRequest> createFormData(
    ...
    print('Token being used: $_authToken');
    ...
    for (var entry in fields.entries) {
      if (entry.value is String && entry.value.toString().startsWith('/')) {
        ...
        final filename = entry.value.toString().split('/').last;
        final mimeType = _getMimeType(filename);
        ...
        request.files.add(multipartFile);
        print(
          'Added file ${entry.key}: $filename with type ${mimeType.toString()}',
        );
      } else {
        request.fields[entry.key] = entry.value.toString();
        print('Added field ${entry.key}: ${entry.value}');
      }
    }
  ...
  Future<String> createMultipartFile(String filePath) async {
    final file = File(filePath);
    final filename = filePath.split('/').last;
    final mimeType = _getMimeType(filename);
    ...
    print('Created multipart file: $filename with type ${mimeType.toString()}');
  ...
  T _handleResponse<T>(http.Response response) {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      if (response.statusCode == 401) {
        ...
        _prefs.clearAll(); // Clear stored data
      }
